## Naveen J AKA [Thevillagehacker](https://twitter.com/thevillagehackr)<br>
[![Gmail Badge](https://img.shields.io/badge/-nvnj1998@gmail.com-c14438?style=flat-square&logo=Gmail&logoColor=white&link=mailto:nvnj1998@gmail.com)](mailto:nvnj1998@gmail.com)
[![Linkedin Badge](https://img.shields.io/badge/-naveenj-blue?style=flat-square&logo=Linkedin&logoColor=white&link=https://www.linkedin.com/in/naveen-jagadeesan/)](https://www.linkedin.com/in/naveen-jagadeesan/)
[![Twitter Follow](https://img.shields.io/twitter/follow/thevillagehackr?style=social)](https://twitter.com/thevillagehackr)
[![Twitter](https://img.shields.io/twitter/url?color=green&label=Download%20CV&style=flat-square&url=https%3A%2F%2Fgithub.com%2Fthevillagehacker%2Fthevillagehacker.github.io%2Fraw%2Fmain%2Fcv.pdf)](https://github.com/thevillagehacker/thevillagehacker.github.io/raw/main/cv.pdf)

*Hey everyone! My name is Naveen, and I'm currently working in the cybersecurity realm as a penetration tester, performing security testing ranging from network penetration, web application assessments to social engineering and physical engagements. Seasoned and fully dependable Security Engineer with exceptional record of competence and discretion. Adept at communicating with other engineers and clients in a clear and understandable manner. Able to maintain the highest standards of confidentiality in handling and protecting sensitive client information.*

## Skills
- Web App and API Penetration Testing
- Windows App Penetration Testing
- Mobile App Penetration Testing
- Wireless Penetration Testing
- SIEM & Log Management
- Internet of Things
- OWASP Top 10
- Bash Scripting
- JavaScript
- GraphQL
- Python

## Work Experience
***SEPTEMBER 2019 - PRESENT KGISL GSS COIMBATORE, INDIA***
#### Information Security Engineer
- Perform Web Application, Mobile Application and Thick client Application Penetration Testing.
- Work with external vendor to perform Penetration tests on network devices, Applications, Servers as necessary.
- Analyze security results, draw conclusions from the result and develop targeted testing as deemed necessary.
- Work with application developers to validate, assess, understand root cause and mitigate vulnerabilities.
- Perform Security review of application designs and source code reviews.
- Perform Security Audit for all network devices in the organization infrastructure.
- Work on improvements for security services, including the continuous enhancement of existing methodology material and supporting assets.
- Provides assistance to system users relative to information systems security matters.
- Develop security standards, policies, automation scripts.
- Develop and implement tools and techniques to perform ongoing security assessments of the environment.
- Perform infrastructure and application penetration tests, as well as physical security review and social engineering tests for our global clients.
- Perform application penetration tests across public and private networks.
- Perform assessments of security awareness training using social engineering.

***2017 - 2018 MGK ENTERPRISE SYSTEMS COIMBATORE, INDIA***
#### IoT Developer - Intern
- Hardware design(microcontroller, sensors, circuit) for the need of client.
- Test the individual I/O systems.
- Performed Penetration Testing on IoT dashboard web and Android applications.
- Participate in the definition of development of IoT roadmaps and supports the technical decisions.
- Working knowledge on IoT Strategy, standards, protocols.
- Hands on experience with constrained devices such as Arduino, RaspberryPI, TI, ARM.
- Work with IoT developer and bussiness to validate, assess and remediate the security Vulnerabilities.
- Help develop an overall IoT and Mobile ecosystem engagement strategy.
- Identify architectural risks and plans to mitigate.

## My Gitbooks
- ***[🛡️ Red Teaming Cheatsheet](https://thevillagehacker.gitbook.io/red-teaming/)***
- ***[✅ Web Pentest Checklist](https://thevillagehacker.gitbook.io/web-pentest-checklist/)***
- ***[🤖 Onliners for Bug Hunting](https://thevillagehacker.gitbook.io/one-liners-for-bug-bounty-hunting/)***
