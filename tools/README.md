...
tag: CVE-XXXX-YYYY          # green pill under nav + research-cve on blog card
...
- Inline dark-theme fallback so the page never flashes unstyled white
...
Plain ` ```text ` / ` ```plaintext ` blocks that *start* like HTTP (`POST /…`, `HTTP/1.1 200 …`) are **auto-promoted** to `language-http`. At runtime, `site.js` colors methods, status codes (2xx green / 4xx amber / 5xx red), headers, and JSON/XML bodies.
